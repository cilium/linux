...
where ``I_loader`` is the loader's instruction stream and ``H_meta`` is the
...
signed by a system keyring, accept a user-keyring signature only if its key is
on an explicit allowlist, and emit a tamper-evident record of every decision so
that even denied attempts are auditable. (Illustrative -- error checking
elided.)

.. code-block:: c

    /* Serials of user-keyring keys we additionally trust. */
    struct {
                 ...
                 struct bpf_token *token, bool kernel)
    {
            ...
            if (verdict != BPF_SIG_VERIFIED)
                    ...
                    ret = -EPERM;                   /* user key not allowlisted */
...
any other value is treated as the serial of a user/session keyring. The keyring
is looked up first, before the signature bytes are examined, so a signature
naming a non-existent keyring is rejected up front, and a failed verification
aborts the load -- so a program that loads successfully with a signature always
has consistent keyring fields recorded.
...
``keyring_serial`` (``s32``)
    Set **only** on a successful verification, to the serial of the asymmetric
    key that actually validated the signature -- and only when that key came
    from a **user/session** keyring. The system keyrings are trusted as a whole
    and expose no per-key serial here, so the serial is ``0`` for builtin,
    secondary and platform signatures, and ``0`` for unsigned programs. In other
    words, a non-zero ``keyring_serial`` is exactly "verified against a
    user-keyring key with this serial".
