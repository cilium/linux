 ...
 *
 * *xattrs* is NULL when the filesystem creating the inode takes no xattrs
 * at creation, in which case there is nothing to claim.
 *
 * Return: 0 on success, a negative value on error.
 */
				     ...
				     const struct bpf_dynptr *value_p)
{
	...
	/*
	 * The suffix has to name something below "bpf.", and initxattrs() puts
	 * the "security." prefix back, so the name the filesystem ends up
	 * storing is that much longer than the suffix.
	 */
	...
	/*
	 * Once initxattrs() has run, the LSM framework releases only ->value
	 * of each claimed slot: the in-tree LSMs all point ->name at a string
	 * constant. This name is built at runtime instead, so it is placed in
	 * the tail of the value's allocation and freed along with it.
	 *
	 * The hook runs inside the transaction creating the inode, so this
	 * cannot reclaim or sleep. A policy that needs the claim to succeed
	 * under memory pressure should keep its labels small.
	 */
