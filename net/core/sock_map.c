...
static void sock_map_del_link(struct sk_psock *psock, void *link_raw)
{
...
static void sock_map_unref(struct sock *sk, void *link_raw)
{
	...
	if (likely(psock)) {
		sock_map_del_link(psock, link_raw);
		sk_psock_put(sk, psock);
	}
}

static bool sk_is_sock_map(struct sock *sk)
{
	// tbd need an identifier :/
	return true;
}
...
void sk_psock_unlink(struct sock *sk, struct sk_psock_link *link)
{
	...
	case BPF_MAP_TYPE_SOCKMAP:
		return sock_map_delete_from_link(link->map, sk, link->link_raw);
	case BPF_MAP_TYPE_SOCKHASH:
		return sock_hash_delete_from_link(link->map, sk, link->link_raw);
	default:
		break;
	}
}
