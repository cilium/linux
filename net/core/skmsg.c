...
static int sk_msg_free_elem(struct sock *sk,
			    struct sk_msg *msg, int which, bool charge)
{
	struct scatterlist *sge = &msg->sg.data[which];
	int len = sge->length;
...
static int __sk_msg_free(struct sock *sk,
			 struct sk_msg *msg, u32 i, bool charge)
{
	struct scatterlist *sge = sk_msg_elem(msg, i);
	int freed = 0;
...
#else
	/* Check sg.size to account for encrypted ring where data
	 * is in-flight so length is non-zero in scatterlist.
	 */
	if (!msg->sg.size)
		return freed;
...
{
	int free = __sk_msg_free(sk, msg, msg->sg.start, true);

	sk_msg_init(msg, false);
	return free;
}
			      ...
			      struct sk_msg *msg, u32 bytes)
{
	...
	int num_elems = sk_msg_elem_used(msg);
	...
	while (bytes > 0) {
		 ...
		 */
		msg->sg.copybreak = 0;
		msg->sg.curr = msg->sg.end;//msg->sg.end - 1;
	}
out:
	/* Revert iov_iter updates, msg will need to use 'trim' later if it also
	 * needs to be cleared.
	 */
	if (rc)
...
int sk_msg_memcopy_from_iter2(struct sock *sk, struct iov_iter *from,
			     struct sk_msg *msg, u16 *to_start, int bytes)
{
	int copy, i, ret;
	void *to;

	for (i = *to_start; i < msg->sg.end; i++) {
		copy = msg->sg.data[i].length;
		to = page_address(sg_page(&msg->sg.data[i])) +
		     msg->sg.data[i].offset;
		if (sk->sk_route_caps & NETIF_F_NOCACHE_COPY)
			ret = copy_from_iter_nocache(to, copy, from);
		else
			ret = copy_from_iter(to, copy, from);
...
EXPORT_SYMBOL_GPL(sk_msg_memcopy_from_iter2);
