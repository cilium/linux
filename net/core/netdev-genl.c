...
{
	struct net_devmem_dmabuf_binding *binding;
	struct net_device *netdev, *bind_dev;
	struct netdev_nl_sock *priv;
	struct device *dma_dev;
	...
	bind_dev = netdev_find_netmem_tx_dev(netdev, &txq_idx);
	if (!bind_dev) {
		err = -EOPNOTSUPP;
