...
enum special_kfunc_type {
	...
	KF_bpf_set_file_xattr,
	KF_bpf_remove_file_xattr,
...
static int specialize_kfunc(struct bpf_verifier_env *env, struct bpf_kfunc_desc *desc, int insn_idx)
{
	...
	} else if (func_id == special_kfunc_list[KF_bpf_set_file_xattr]) {
		if (bpf_lsm_has_d_inode_locked(prog))
			addr = (unsigned long)bpf_set_file_xattr_locked;
	} else if (func_id == special_kfunc_list[KF_bpf_remove_file_xattr]) {
		if (bpf_lsm_has_d_inode_locked(prog))
			addr = (unsigned long)bpf_remove_file_xattr_locked;
