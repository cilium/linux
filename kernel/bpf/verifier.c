			    ...
			    int value_regno, bool strict_alignment_once, bool is_ldsx)
{
	...
	if (!err && size < BPF_REG_SIZE && value_regno >= 0 && t == BPF_READ &&
		...
		if (!is_ldsx) {
			 ...
			 * to a scalar it is linked with by id (e.g. a
			 * zero-extending fill of the same spilled stack slot),
			 * so drop the shared id in that case, mirroring the
			 * BPF_MOVSX handling in check_alu_op(). Otherwise
			 * sync_linked_regs() could later propagate a wrong range.
			 */
			bool no_sext = reg_umax(&regs[value_regno]) < (1ULL << (size * BITS_PER_BYTE - 1));
