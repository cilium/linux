...
static int check_indirect_jump(struct bpf_verifier_env *env, struct bpf_insn *insn)
{
	...
	/*
	 * The map the register points to is not necessarily the one the CFG
	 * derived the jump table of this subprogram from, so the targets have
	 * to be confined here as well. A gotox that leaves its subprogram runs
	 * the epilogue of another one, which pops a different set of callee
	 * saved registers than the prologue of the current frame pushed.
	 */
	...
	for (i = 0; i < n; i++) {
		...
		if (target < subprog_start || target >= subprog_end)
			return reject_gotox_out_of_subprog(env, target, subprog_start,
							   subprog_end);
		/*
		 * 'jt' holds the edges bpf_insn_successors() reported for this
		 * gotox, so it is what the loop and SCC analysis walked. A
		 * target outside of it has no edge in the CFG and must not be
		 * pushed for verification.
		 */
