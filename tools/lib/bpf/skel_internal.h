...
static inline int bpf_load_and_run(struct bpf_load_and_run_opts *opts)
{
	 ...
	 * kernel at load time (see attr.fd_array_cnt below), which requires
	 * the map to be frozen so the bytes cannot change before the loader
	 * runs.
	 */
	...
	/*
	 * Bind the metadata map through fd_array so the kernel folds its
	 * frozen contents into the signature: it verifies insns || metadata.
	 */
	if (opts->signature)
		attr.fd_array_cnt = 1;
#else
