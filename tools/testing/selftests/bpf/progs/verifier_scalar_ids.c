 ...
 * sign-extending load (BPF_MEMSX) into r4 and once via a zero-extending load
 * (BPF_MEM) into r5. coerce_reg_to_size_sx() gives r4 a different value than
 * the spilled/zero-extended siblings, so r4 must not keep the shared scalar
 * id. Otherwise the later 'if r5 == 0x80000000' refines r4 through
 * sync_linked_regs() to a known 0x80000000, while at runtime r4 is the
 * sign-extended 0xffffffff80000000. The test turns that discrepancy into an
 * out-of-bounds map value access (r4 >> 63 is believed 0 but is 1 at
 * runtime), which must be rejected.
 */
SEC("socket")
__description("MEMSX and MEM stack fills must not share a scalar id")
