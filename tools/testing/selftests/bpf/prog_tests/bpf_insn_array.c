...
static void check_gotox_jt_dead_subprogs(void)
{
	...
	opts.fd_array_cnt = ARRAY_SIZE(map_fd);
	opts.log_buf = log;
	opts.log_size = GOTOX_LOG_SZ;
	opts.log_level = 1;
	prog_fd = bpf_prog_load(BPF_PROG_TYPE_XDP, NULL, "GPL", insns,
...
{
	struct bpf_insn xlated[128] = {};
	struct bpf_prog_info info = {};
	__u32 len = sizeof(info), cnt, i;

	info.xlated_prog_insns = ptr_to_u64(xlated);
	info.xlated_prog_len = sizeof(xlated);
	if (!ASSERT_OK(bpf_prog_get_info_by_fd(prog_fd, &info, &len), "bpf_prog_get_info_by_fd"))
		return -1;

	cnt = info.xlated_prog_len / sizeof(*insn);
	for (i = 0; i < cnt && i < ARRAY_SIZE(xlated); i++)
