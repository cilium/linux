...
static bool try_load_gotox_prog(__u32 gotox_cnt, bool target_exit, char *log, int *err)
{
...
static void check_too_many_gotox_edges(void)
{
	...
	if (try_load_gotox_prog(gotox_cnt, true, log, &err))
...
/*
 * Without the exit target the jump table holds the gotox run alone, so the
 * program sits exactly at the limit and must still load.
 */
