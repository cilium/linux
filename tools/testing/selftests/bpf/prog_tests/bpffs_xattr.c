...
static void test_prefixes(void)
{
	...
	err = getxattr(dir, "security.baz", buf, sizeof(buf));
	ASSERT_EQ(err, sizeof(val_a), "security_survives_remove");
out:
	umount_bpffs(dir);
}
...
static void test_unsupported_prefix(void)
{
	...
	/* user.* is not supported on bpffs, like sysfs. */
	err = setxattr(dir, "user.foo", val_a, sizeof(val_a), XATTR_CREATE);
	ASSERT_EQ(err, -1, "set_user_ret");
	ASSERT_EQ(errno, EOPNOTSUPP, "set_user_errno");
...
static void test_flags(void)
{
	...
	err = setxattr(dir, "trusted.x", val_a, sizeof(val_a), XATTR_REPLACE);
	ASSERT_EQ(err, -1, "replace_missing_ret");
	ASSERT_EQ(errno, ENODATA, "replace_missing_errno");

	err = removexattr(dir, "trusted.x");
	ASSERT_EQ(err, -1, "remove_missing_ret");
	ASSERT_EQ(errno, ENODATA, "remove_missing_errno");

	ASSERT_OK(setxattr(dir, "trusted.empty", "", 0, XATTR_CREATE), "set_empty");
	err = getxattr(dir, "trusted.empty", buf, sizeof(buf));
	ASSERT_EQ(err, 0, "get_empty_len");

	ASSERT_OK(setxattr(dir, "trusted.x", val_a, sizeof(val_a), XATTR_CREATE),
		  "create_x");
	err = setxattr(dir, "trusted.x", val_b, sizeof(val_b), XATTR_CREATE);
	ASSERT_EQ(err, -1, "create_existing_ret");
	ASSERT_EQ(errno, EEXIST, "create_existing_errno");

	ASSERT_OK(setxattr(dir, "trusted.x", val_b, sizeof(val_b), XATTR_REPLACE),
		  "replace_existing");

	err = getxattr(dir, "trusted.x", buf, sizeof(buf));
	ASSERT_EQ(err, -1, "get_too_small_ret");
	ASSERT_EQ(errno, ERANGE, "get_too_small_errno");

	err = getxattr(dir, "trusted.x", NULL, 0);
...
static void test_inode_types(void)
{
	...
	/* mount root inode */
	ASSERT_OK(setxattr(dir, "trusted.root", val_a, sizeof(val_a), XATTR_CREATE),
		  "root_set");
	err = getxattr(dir, "trusted.root", buf, sizeof(buf));
	...
	if (!ASSERT_OK(mkdir(child, 0755), "mkdir"))
		goto out;
	err = getxattr(child, "trusted.root", buf, sizeof(buf));
	ASSERT_EQ(err, -1, "no_inherit_ret");
	ASSERT_EQ(errno, ENODATA, "no_inherit_errno");
	ASSERT_OK(setxattr(child, "trusted.dir", val_a, sizeof(val_a), XATTR_CREATE),
		  "dir_set");
	...
	if (!ASSERT_OK(bpf_obj_pin(map_fd, map_path), "map_pin"))
		goto out;
	ASSERT_OK(setxattr(map_path, "trusted.map", val_a, sizeof(val_a),
	...
	if (!ASSERT_OK(bpf_obj_pin(bpf_program__fd(skel->progs.raw_tp_prog),
				   prog_path), "prog_pin"))
		goto out;
	ASSERT_OK(setxattr(prog_path, "trusted.prog", val_a, sizeof(val_a),
	...
	if (!ASSERT_OK(bpf_obj_pin(bpf_link__fd(skel->links.raw_tp_prog),
				   link_path), "link_pin"))
		goto out;
	ASSERT_OK(setxattr(link_path, "trusted.link", val_a, sizeof(val_a),
...
static void test_repin_same_path(void)
{
	...
	if (!ASSERT_OK(bpf_obj_pin(map_a, path), "pin_a"))
		goto out;
	ASSERT_OK(setxattr(path, "trusted.k", val_a, sizeof(val_a), XATTR_CREATE),
		  "set_a");
	...
	err = getxattr(path, "trusted.k", buf, sizeof(buf));
...
static void test_umount_lifetime(void)
{
	...
	ASSERT_OK(setxattr(path, "trusted.k", val_a, sizeof(val_a), XATTR_CREATE),
		  "set");
	/* Label the mount root too; its xattrs must not survive umount. */
	ASSERT_OK(setxattr(dir, "trusted.root", val_a, sizeof(val_a), XATTR_CREATE),
		  "set_root");
	err = getxattr(dir, "trusted.root", buf, sizeof(buf));
	ASSERT_EQ(err, sizeof(val_a), "root_xattr_present");
