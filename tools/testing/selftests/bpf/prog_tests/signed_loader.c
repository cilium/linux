		       ...
		       __u32 fd_array_cnt)
{
	...
	/* When non-zero, the kernel scans fd_array for the bound exclusive
	 * maps and folds their contents into the signature; a signed loader
	 * must do this or the load is rejected.
	 */
			  ...
			  bool get_hash, void *ctx, __u32 ctx_sz, bool *loader_ran)
{
	...
	*loader_ran = false;
	...
	if (sig) {
		...
		/* A signed loader must fold its metadata: the signature is
		 * expected to cover insns || metadata.
		 */
		attr.fd_array_cnt = 1;
	}
	memcpy(attr.prog_name, "__loader.prog", sizeof("__loader.prog"));
...
static void signature_authenticates_insns(void)
{
	...
	/* The signature covers insns || metadata -- the bytes the kernel folds
	 * and reconstructs at load time -- so tampering with either is caught.
	 */
...
static void lsm_signature_verdict(void)
{
	...
	/*
	 * A signed loader that does not fold its metadata is rejected: it uses
	 * the exclusive metadata map, but this signature (instructions only)
	 * does not cover it. The signature itself is valid, so the admission
	 * hook still observes BPF_SIG_VERIFIED and the validating keyring -- but
	 * the load fails the binding check, so there is no "signed but
	 * payload-unverified" state.
	 */
	...
	/*
	 * Metadata-bound load: the signature covers insns || metadata and the
	 * map is bound through fd_array_cnt, so the load reaches BPF_SIG_VERIFIED.
	 */
	{
		unsigned char *buf;
