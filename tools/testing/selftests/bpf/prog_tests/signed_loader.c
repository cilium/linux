...
/* Well-known bpf_attr.keyring_id values; the kernel does not export these. */
#define BPF_KEYRING_BUILTIN	0
...
static void bpf_keyring_sealed(void)
{
	...
	if (gen_loader_fixture_init(&f) == 0) {
		 ...
		 * signature bytes are examined. An unsealed but unprovisioned
		 * keyring behaves the same way, since it is only consulted
		 * once it is both non-empty and restricted.
		 */
		fd = load_loader(f.gopts.insns, f.gopts.insns_sz, -1, junk,
 ...
 * This pins that end to end rather than by inspection.
 */
static void mldsa_signed_load(void)
{
	...
	/*
	 * ML-DSA key types need openssl 3.5+, which is not something the test
	 * can arrange, so skip when it is missing. The kernel side is covered
	 * by CONFIG_CRYPTO_MLDSA in the selftest config and is not a skip.
	 */
