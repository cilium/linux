		       ...
		       __u32 excl_map_fd)
{
	...
	/* When set, the signature is expected to cover insns || metadata and
	 * the load reaches the BPF_SIG_VERIFIED_METADATA verdict.
	 */
	attr.excl_map_fd = excl_map_fd;
	memcpy(attr.prog_name, "__loader.prog", sizeof("__loader.prog"));
	fd = syscall(__NR_bpf, BPF_PROG_LOAD, &attr,
		     offsetofend(union bpf_attr, excl_map_fd));
...
static void lsm_signature_verdict(void)
{
	 ...
	 * map is handed to the kernel via excl_map_fd, so the load reaches the
	 * stronger BPF_SIG_VERIFIED_METADATA verdict.
	 */
	{
		unsigned char *buf;
		...
		lsm->bss->seen = 0;
		prog_fd = load_loader(f.gopts.insns, f.gopts.insns_sz, map_fd, sig,
				      msig_sz, KEY_SPEC_SESSION_KEYRING, map_fd);
		close(map_fd);
		map_fd = -1;
		...
		ASSERT_EQ(lsm->bss->sig_verdict, BPF_SIG_VERIFIED_METADATA,
 ...
 * excl_map_fd, rather than the loader checking from within BPF. Sign that
