...
static const struct option long_options[] = {
	...
	{"txmsg_start_cut", required_argument,	NULL, 'w'},
	{"txmsg_cut",   required_argument,	NULL, 'x'},
		    ...
		    struct sockmap_options *opt)
{
	...
	if (tx) {
		...
		float total_bytes, txmsg_cut_total;
		int fd_flags = O_NONBLOCK;
		struct timeval timeout;
		fd_set w;

		fcntl(fd, fd_flags);
		/* Account for cut bytes noting each iteration of apply will
		 * call msg_cut_data helper so we need to account for this
		 * by calculating the number of apply iterations. Note user
		 * of the tool can create cases where no data is sent by
		 * manipulating cut/apply/pull/etc. For example txmsg_apply 1
		 * with txmsg_cut 1 will try to apply 1B at a time but each
		 * iteration will then cut 1B so no data will ever be sent.
		 * This is really only useful for testing edge cases in code
		 * paths.
		 */
		total_bytes = (float)iov_count * (float)iov_length * (float)cnt;
		txmsg_cut_total = txmsg_cut;
		if (txmsg_apply)
			txmsg_cut_total *= (total_bytes / txmsg_apply);
		total_bytes -= txmsg_cut_total;
		...
		while (s->bytes_recvd < total_bytes) {
			...
			if (slct == -1) {
				...
				if (opt->verbose)
					fprintf(stderr, "unexpected timeout: recved %zu/%f cut_total %f\n", s->bytes_recvd, total_bytes, txmsg_cut_total);
				errno = -EIO;
				clock_gettime(CLOCK_MONOTONIC, &s->end);
				goto out_errno;
			}
...
run:
	...
	if (tx_prog_fd) {
		...
		if (txmsg_start_cut) {
			i = 4;
			err = bpf_map_update_elem(map_fd[5],
						  &i, &txmsg_start_cut, BPF_ANY);
			if (err) {
				fprintf(stderr,
					"ERROR: bpf_map_update_elem %i@%i (txmsg_start_cut):  %d (%s)\n",
					txmsg_start_cut, i, err, strerror(errno));
				goto out;
			}
		} else {
			i = 4;
		...
		if (txmsg_cut) {
			...
			if (err) {
				fprintf(stderr,
					"ERROR: bpf_map_update_elem %i@%i (txmsg_cut):  %d (%s)\n",
					txmsg_cut, i, err, strerror(errno));
				goto out;
			}
		} else {
			i = 5;
...
static void test_options(char *options)
{
	...
	if (txmsg_start_cut) {
		snprintf(tstr, OPTSTRING, "cut (%d,%d),",
			 txmsg_start_cut, txmsg_start_cut + txmsg_cut);
		strncat(options, tstr, OPTSTRING);
	}
	if (txmsg_ingress)
...
static int test_start_end(int cgrp)
{
	...
	/* Test cut data in middle of cork */
	for (i = 99; i <= 1600; i += 500) {
		txmsg_start_cut = 10;
		txmsg_cut = i;
		err = test_exec(cgrp, &opt);
	...
	/* Test cut with cork pulling last sg entry */
	txmsg_start_cut = 1500;
	txmsg_cut = 1600;
	err = test_exec(cgrp, &opt);
	if (err)
		goto out;
	...
	/* Test cut of single byte in last page */
	txmsg_start_cut = 1111;
	txmsg_cut = 1112;
	err = test_exec(cgrp, &opt);
	if (err)
		goto out;
	...
	/* Test cut with start > data */
	txmsg_start_cut = 1601;
	txmsg_cut = 1;
	err = test_exec(cgrp, &opt);
	if (err)
		goto out;

	/* Test cut with cut range > data */
	txmsg_start_cut = 1599;
	txmsg_cut = 10;
	err = test_exec(cgrp, &opt);
out:
	txmsg_start = 0;
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, ":dhvc:r:i:l:t:p:q:",
		...
		case 'q':
			...
			break;
		case 'w':
			txmsg_start_cut = atoi(optarg);
			break;
		case 'x':
			txmsg_cut = atoi(optarg);
			break;
		case 'a':
