 ...
 * BPF_ATOMIC reads the value at its memory operand into, or -1 if there is no
 * such register. A load-acquire reads into dst_reg, a BPF_CMPXCHG into r0 and
 * every other read-modify-write carrying BPF_FETCH into src_reg. That is the
 * register a BPF_PROBE_ATOMIC has to clear when the access faults. Like
 * bpf_atomic_is_load_acq(), @insn is not assumed to be a BPF_ATOMIC here.
 */
